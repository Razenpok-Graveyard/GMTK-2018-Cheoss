...
{
    public static IEnumerable<float> Linear(float from, float to, float time)
    {
        return Ease(from, to, time, fraction => fraction);
    }

    public static IEnumerable<float> InSquare(float from, float to, float time)
    {
        return Ease(from, to, time, fraction => Mathf.Pow(fraction, 2f));
    }

    public static IEnumerable<float> OutSquare(float from, float to, float time)
    {
        return Ease(from, to, time, fraction => Mathf.Pow(fraction, 1 / 2f));
    }
